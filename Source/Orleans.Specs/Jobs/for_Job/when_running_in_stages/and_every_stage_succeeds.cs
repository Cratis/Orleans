// Copyright (c) Cratis. All rights reserved.
// Licensed under the MIT license. See LICENSE file in the project root for full license information.

namespace Cratis.Orleans.Jobs.for_Job.when_running_in_stages;

public class and_every_stage_succeeds : given.a_job_with_three_stages
{
    async Task Because()
    {
        await _job.Start(new());
        await Succeed(_first);
        await Succeed(_secondA);
        await Succeed(_secondB);
        await Succeed(_third);
    }

    [Fact] void should_start_the_step_in_the_third_stage() => ShouldHaveStarted(_thirdStep);
    [Fact] void should_complete_the_job_successfully() => _job.CurrentState.Status.ShouldEqual(JobStatus.CompletedSuccessfully);
    [Fact] void should_count_every_step_as_successful() => _job.CurrentState.Progress.SuccessfulSteps.ShouldEqual(4);
}
