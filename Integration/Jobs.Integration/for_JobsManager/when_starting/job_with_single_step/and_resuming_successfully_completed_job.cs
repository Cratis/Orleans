...
using context = Cratis.Orleans.Jobs.Integration.for_JobsManager.when_starting.job_with_single_step.and_resuming_successfully_completed_job.context;
...
public class and_resuming_successfully_completed_job(context context) : Given<context>(context)
{
    public class context : a_jobs_manager
    {
        ...
        async Task Because()
        {
            ...
            await JobStorage.WaitTillJobMeetsPredicate(JobId, state => state.Status == JobStatus.CompletedSuccessfully && state.Progress.IsCompleted, TimeSpan.FromSeconds(20));
            await JobsManager.Resume(JobId);
            CompletedJobState = await JobStorage.WaitTillJobMeetsPredicate(JobId, state => state.Status == JobStatus.CompletedSuccessfully && state.Progress.IsCompleted, TimeSpan.FromSeconds(20));
